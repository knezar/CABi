...
class FirebaseAPIClient {
    ...
    private init() {}
    func login(with email: String, and password: String, completion: @escaping (AuthDataResult?, Error?) -> Void) {
...
//                        self.DownloadImage(childRef: snap.key)
    ...
    func createTasks(task: Task, userEmail: String) {
        let ref = Database.database().reference().child("driverTasks").child(userEmail)
        print(userEmail)
